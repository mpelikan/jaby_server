...
( function ( window, document, exportName ) {
	...
	var communiqué = ( function () {
		...
		/**
		 * Finds and stores references to DOM elements which are required by the presentation.
		...
		/**
		 * Steps from the current point in the presentation to the message which matches the specified index.
		 *
		...
		function message( i ) {
			...
			//	Update the visibility of messages now that the index has changed
		...
		/**
		 * Optimization method; hide all messages that are far away from the present message.
		 */
		 ...
		 *
		 * @param {HTMLElement} message If specified, the returned index will be for this message
		...
		/**
		 * Handles mouse wheel scrolling, throttled to avoid skipping multiple messages.
		 */
		...
		function isLastMessage() {
			...
			if ( currentMessage ) {
				//	Does this message have next sibling?
		...
		function getMessage( index ) {
			...
			var message = document.querySelectorAll( MESSAGES_SELECTOR )[ index ];
		...
		return {
			...
			//	Returns the index of the current, or specified, message
			getIndex: getIndex,
