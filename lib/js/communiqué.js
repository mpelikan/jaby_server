...
( function ( window, document, exportName ) {
	...
	var communiqué = ( function () {
		...
		var config = {
			...
			//	Transition style for full page message backgrounds
		...
		function enableRollingLinks() {
			...
			if ( features.transforms3d && !( "msPerspective" in document.body.style ) ) {
				var anchors = document.querySelectorAll( MESSAGES_SELECTOR + " a:not(.image)" );
		...
		function disableRollingLinks() {
			...
			var anchors = document.querySelectorAll( MESSAGES_SELECTOR + " a.roll" );
		...
		function updateMessagesVisibility() {
			...
			if ( messagesLength ) {
				...
				//	The number of steps away from the present message that will be visible
		...
		function availableRoutes() {
			...
			var routes = {
				...
				right: index < messages.length - 1 || config.loop
			};
