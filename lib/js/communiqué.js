...
( function ( window, document, exportName ) {
	...
	var communiqué = ( function () {
		...
		var HORIZONTAL_SLIDES_SELECTOR = ".communiqué .slides>section";
		...
		/**
		 * Starts up communiqué.js by binding input events and navigating
		...
		function createBackgrounds() {
			...
			toArray( document.querySelectorAll( HORIZONTAL_SLIDES_SELECTOR ) ).forEach( function ( slideh ) {
				...
				_createBackground( slideh, dom.background );
		...
		function slide( h ) {
			...
			// Find the current horizontal slide
		...
		function updateSlidesVisibility() {
			...
			var horizontalSlidesLength = horizontalSlides.length;
			var distanceX;
		...
		function updateBackground() {
			...
			var horizontalFuture = config.rtl ? "past" : "future";
		...
		function removeSlide( index, id ) {
			...
			if ( index === -1 ) {
				...
				if ( index > getIndices().h && target ) {
					dom.slides.removeChild( target );
				}
				else {
					if ( index < getIndices().h && target ) {
						dom.slides.removeChild( target );
						slide( getIndices().h - 1 );
					}
					else {
						if ( index === getIndices().h && target ) {
							if ( index === 0 ) {
