...
( function ( window, document, exportName ) {
	...
	jaby.start = function start() {
		...
		setTimeout( deferredStart.bind( this ), 1 );
	...
	jaby.onUserInput = function onUserInput( event ) {
		...
		if ( source === this.questionInput ) {
			if ( source.value ) {
				if ( event.keyCode === 13 ) {
					answer = source.getAttribute( "id" );
					okButton = document.getElementById( "submit_" + answer );
					okButton.click();
	...
	jaby.requestAnimation = function requestAnimation() {
	...
	jaby.turnOnAnimation = function turnOnAnimation() {
		if ( !this.animationTimer ) {
			this.animationTimer = setInterval( this.requestAnimation.bind( this ), 100 );
		}
	};
	...
	jaby.turnOnPolling = function turnOnPolling() {
		...
		if ( !this.pollingTimer ) {
			this.pollingTimer = setInterval( this.status.bind( this ), 10000 );
		}
	};
	...
	jaby.setPowerColor = function setPowerColor( color ) {
		this.inner_power.setAttribute( "fill", color );
	};

	jaby.setOrbColor = function setOrbColor() {
		if ( this.on ) {
	...
	jaby.retractMessage = function retractMessage( id ) {
		var message;
		...
		if ( id ) {
			...
			if ( message ) {
				message.dataset.expires = new Date();
	...
	jaby.askQuestion = function askQuestion( id, question, answers ) {
		...
		function createQuestion() {
			...
			function getAnswerFunction( answer ) {
				if ( !answer ) {
					return function () {
						var answer = "answer_" + answers;
						var textAnswerElement = document.getElementById( answer );
						var textAnswer = textAnswerElement ? textAnswerElement.value : "";
						self.sendAnswer( id, textAnswer );
						return false;
					};
				}
				else {
					return function () {
						self.sendAnswer( id, answer );
						return false;
					};
				}
			}
			...
			var i, numAnswers, answer, answerFunction;
			...
			if ( answers ) {
				if ( Object.prototype.toString.call( answers ) === "[object Array]" ) {
					...
					for ( i = 0; i < numAnswers; i++ ) {
						answer = answers[ i ];
						answerFunction = getAnswerFunction( answer.id );
						buttonElement = document.createElement( "a" );
						buttonElement.addEventListener( "click", answerFunction, false );
					...
					if ( typeof answers === "string" ) {
						...
						buttonElement = document.createElement( "a" );
						buttonElement.setAttribute( "id", "submit_" + answer );
						answerFunction = getAnswerFunction();
						buttonElement.addEventListener( "click", answerFunction, false );
						buttonElement.setAttribute( "class", "btn" );
						...
						self.questionInput = inputElement;
					}
				}
			}
	...
	jaby.userRequest = function userRequest() {
		...
		var data = {
			message: "The user clicked on the orb."
		};
	...
	jaby.setupSocketIO = function setupSocketIO() {
		...
		this.socket.on( "retract", function ( message ) {
			self.retractMessage.call( self, message.id );
		} );
