...
( function ( window, document, exportName ) {
	...
	jaby.onStatus = function onStatus( data ) {
		if ( data && data.message ) {
			...
			this.setActivityLoad( data.activityLoad );
	...
	jaby.setActivityLoad = function setActivityLoad( load ) {
		if ( load && typeof load === "number" && load > 0 && load <= 10 ) {
			this.pulseDirection = load;
			if ( this.pulse > ( this.pulseMax / 2 ) ) {
				this.pulse = this.pulseMax;
				this.pulseDirection = this.pulseDirection * -1;
			}
			else {
				this.pulse = 0;
			}
	...
	jaby.doLeft = function doLeft() {
		...
		if ( previousSlide ) {
			if ( !previousSlide.hasAttribute( "data-remove" ) ) {
				Reveal.prev();
			}
		}
	};
	...
	jaby.setupReveal = function setupReveal() {
		...
		Reveal.initialize( {
			...
			dependencies: [
				{
					src: "third-party/reveal.js/lib/js/classList.js",
					condition: function () {
						return !document.body.classList;
					}
				},
				{
					src: "third-party/reveal.js/plugin/markdown/marked.js",
					condition: function () {
						return !!document.querySelector( "[data-markdown]" );
					}
				},
				{
					src: "third-party/reveal.js/plugin/markdown/markdown.js",
					condition: function () {
						return !!document.querySelector( "[data-markdown]" );
					}
				}
