...
( function ( window, document, exportName ) {
	...
	jaby.addEventListeners = function addEventListeners() {
		...
		this.dom.wrapper.addEventListener( "touchstart", this.onTouchStart.bind( this ), false );
		this.dom.wrapper.addEventListener( "touchmove", this.onTouchMove.bind( this ), false );
		this.dom.wrapper.addEventListener( "touchend", this.onTouchEnd.bind( this ), false );

		//	Support pointer-style touch interaction as well
		if ( window.navigator.msPointerEnabled ) {
			this.dom.wrapper.addEventListener( "MSPointerDown", this.onPointerDown.bind( this ), false );
			this.dom.wrapper.addEventListener( "MSPointerMove", this.onPointerMove.bind( this ), false );
			this.dom.wrapper.addEventListener( "MSPointerUp", this.onPointerUp.bind( this ), false );
		}
	...
	jaby.onTouchStart = function onTouchStart( event ) {
	...
	jaby.onTouchMove = function onTouchMove( event ) {
		...
		if ( !this.touch.captured ) {
			...
			if ( event.touches.length === 1 && this.touch.startCount !== 2 ) {
				...
				deltaX = currentX - this.touch.startX;
				...
				if ( deltaX > this.touch.threshold && Math.abs( deltaX ) > Math.abs( deltaY ) ) {
					...
					if ( deltaX < -this.touch.threshold && Math.abs( deltaX ) > Math.abs( deltaY ) ) {
						...
						if ( deltaY > this.touch.threshold ) {
							this.touch.captured = true;
							//	navigateUp();
						}
						else {
							if ( deltaY < -this.touch.threshold ) {
								this.this.touch.captured = true;
								//	navigateDown();
							}
						}
					}
	...
	jaby.onTouchEnd = function onTouchEnd( /* event */) {

		this.touch.captured = false;

	};

	jaby.onPointerDown = function onPointerDown( event ) {
	...
	jaby.onPointerMove = function onPointerMove( event ) {
	...
	jaby.onPointerUp = function onPointerUp( event ) {
	...
	jaby.setupTimers = function setupTimers() {
		...
		if ( !this.cleanupTimer ) {
			this.cleanupTimer = setInterval( this.cleanupMessages.bind( this ), 5000 );
		}
