...
( function () {
	...
	jaby.core_radius = parseInt( jaby.inner_core.getAttribute( "r" ), 10 );
	...
	jaby.initialize = function initialize() {
		...
		this.inputMessage = document.getElementsByClassName( "inputMessage" )[ 0 ];
	...
	jaby.status = function status() {
		...
		function doStatus() {
			...
			self.socket.emit( "status", context, function () {
				self.onStatus.apply( self, arguments );
			} );
		}
	...
	jaby.setupTouch = function touch() {
		...
		if ( self.content ) {
			self.hideMessageBanner( "panleft" );

			hammertime = new Hammer( this.content );
