...
( function ( window, document, exportName, undefined ) {
	...
	jaby.hideMessageBanner = function hideMessageBanner( direction ) {
		...
		function isVerticalDirection( direction ) {
			return direction === "panup" || direction === "swipeup" || direction === "pandown" || direction === "swipedown";
		}

		if ( direction === "tap" || isVerticalDirection( direction ) ) {
			this.content.classList.add( "vertical_message" );
			this.content.classList.remove( "horizontal_message" );
			this.content.style.left = "10%";
			this.content.style.top = ( direction === "pandown" || direction === "swipedown" ) ? "110%" : "-80%";
		}
		else {
			this.content.classList.remove( "vertical_message" );
			this.content.classList.add( "horizontal_message" );
			this.content.style.top = "10%";
			this.content.style.left = ( direction === "panright" || direction === "swiperight" ) ? "110%" : "-80%";
		}
	...
	jaby.setupEvents = function setupEvents() {
		...
		function onchange( e ) {
			...
			if ( e.type in eventMap ) {
				document.body.className = eventMap[ e.type ];
			}
			else {
				document.body.className = self[ hidden ] ? "hidden" : "visible";
			}
		}

		function windowChange( e ) {
			if ( previousWindowChange !== e.type ) {
				if ( e.type === "blur" ) {
			...
			previousWindowChange = e.type;
		}

		var self = this;
		var hidden = "hidden";
		var previousWindowChange;
		...
		addEvent( document, "blur", windowChange );
		addEvent( document, "focus", windowChange );
